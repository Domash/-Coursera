...
public:
  ...
  AirportCounter(TIterator begin, TIterator end) : AirportCounter(){
    for(TIterator it = begin; it != end; ++it) {
      size_t curr = static_cast<size_t>(*it);
      db[curr]++;
    }
  }

  size_t Get(TAirport airport) const {
    size_t curr = static_cast<size_t>(airport);
    return db[curr];
  }
...
private:
  std::array<int, static_cast<size_t>(TAirport::Last_)> db;
  const size_t len = static_cast<size_t>(TAirport::Last_);
};
...
void TestMoscow() {
  ...
  // cout << airport_counter.Get(MoscowAirport::VKO) << endl;
  // cout << airport_counter.Get(MoscowAirport::SVO) << endl;
  // cout << airport_counter.Get(MoscowAirport::DME) << endl;
  // cout << airport_counter.Get(MoscowAirport::ZIA) << endl;
...
void TestMostPopularAirport() {
  ...
  for (int day = 0; day < days_to_explore; ++day) {
    ...
    for (const auto& it : dayly_flight_report) {
      counter.Insert(it.first);
